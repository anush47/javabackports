...
# Hive build: mvn clean install -DskipTests
# Note: Hive builds can be memory intensive and slow.
# Adding -Dmaven.javadoc.skip=true to save time.
# Adding -B for batch mode (no color/interactive).
