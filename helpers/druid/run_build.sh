...
docker run --rm \
    ...
    -w /repo \
    ${BUILDER_IMAGE_TAG} \
    bash -c "cat web-console/pom.xml && mvn clean install -DskipTests -Ddruid.console.skip=true -Dskip.npm -Dskip.installnodenpm -Dmaven.javadoc.skip=true -Dcheckstyle.skip=true -Dpmd.skip=true -Dforbiddenapis.skip=true -Denforcer.skip=true -Drat.skip=true -T 1C -pl '!:distribution'" \
    || BUILD_EXIT_CODE=$?
