...
IMAGE_TAG="${IMAGE_TAG:-iceberg-${BUILD_TYPE}-${COMMIT_SHA:0:7}}"
...
if ${DOCKER_CMD} run --rm \
    ...
    rsync -a --include='*/' --include='TEST-*.xml' --exclude='*' /repo/ /repo/build_outputs/build/ || echo 'Rsync failed'; \
    echo '--- Test results copied ---'; \
    find /repo/build_outputs -name 'TEST-*.xml' | head -20; \
